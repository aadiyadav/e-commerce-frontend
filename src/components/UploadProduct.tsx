...
const UploadProduct: React.FC<UploadProductProps> = ({ onClose }) => {
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    const resData = await fetch(Api.uploadProduct.url, {
  ...
  return (
    <div className="fixed top-6 left-0 bottom-0 right-0 w-full h-full flex justify-center items-center bg-gray-200 z-10">
      <div className="bg-white p-4 rounded w-full max-w-xl h-full max-h-[60%] overflow-hidden border">
        <div className="flex justify-between text-2xl">
          <div className="text-lg font-bold px-4 font-poppins">Add your Product Details:</div>
          <IoIosCloseCircle onClick={onClose} className="cursor-pointer hover:text-red-600 transition-all" />
        </div>
        <form className="pb-8 my-4 -mr-2 text-base font-dmsans px-4 grid overflow-y-scroll h-[90%] gap-y-5" onSubmit={handleSubmit}>
          <input
            type="text"
          ...
          <label htmlFor="uploadImage">
            <div className="w-full h-32 border rounded-md flex flex-col items-center justify-center cursor-pointer">
              <MdUpload size={40} className="text-red-400" />
          ...
          <div>
            {productDetails.productImage[0] ? (
              <div className="flex items-center gap-x-3">
                {productDetails.productImage.map((el, index) => (
                  <div className="relative group" key={index}>
                    <img src={el} alt="el" className="h-28 cursor-pointer border p-2 rounded-xs" onClick={() => { setOpenFullScreenImage(true); setFullScreenImage(el); }} />
                    <div className="absolute bottom-0 right-0 cursor-pointer text-white p-0.5 m-1 bg-red-500 border rounded-full hidden group-hover:block" onClick={() => handleDeleteProductImage(index)}>
                      <MdDelete />
                    </div>
                  </div>
                ))}
              ...
              <p className="text-red-500 -mt-4">* Please upload product image *</p>
            )}
          </div>
